...
# 11/19/19 'sensitivity test' using seer9 data alone (test if due to demographic changes across registries)
...
siteTable=read.csv(paste0(dir_data,'siteTable',tag,'.csv'),stringsAsFactors = F) %>% data.table()
...
# Compute the corr
for(sex in 1:2){
  TDA=read.csv(paste0(dir_data, 'table_by.sites_stdInciRate_a',min(ages),'-',max(ages),'_',sexes[sex],tag,'.csv'))
  tda=TDA[,-1]
  ...
  write.csv(corr,paste0(dir_proj,'table_by.sites_corr_a',min(ages),'-',max(ages),'_',sexes[sex],tag,'.csv'),row.names = T)
  
}

sex=1  # 1 (for men) and 2 (for women)
da.inci=read.csv(paste0(dir_data,'table_by.sites_stdInciRate_a',min(ages),'-',max(ages),'_',sexes[sex],tag,'.csv'))
corr=read.csv(paste0(dir_proj,'table_by.sites_corr_a',min(ages),'-',max(ages),'_',sexes[sex],tag,'.csv'),row.names = 1)
...
pdf(paste0(dir_proj,'fig_cluster_',method,'_cut',cut.off,'_',sexes[sex],'_a',min(ages),'-',max(ages),tag,'.pdf'),width = 6,height=length(bigtrees)*ifelse(length(bigtrees)>2,1.5,ifelse(length(bigtrees)==2,1.8,3)))
par(mfcol=c(length(bigtrees),2),lheight=.7,xpd=T,mar=c(0,0,0,4.1),oma=c(2.5,2.5,1.5,.5),mgp=c(1.3,.3,0),tck=-.02,cex=.8,cex.lab=.9,cex.axis=.9)
...
for(tr in 1:length(bigtrees)){ # plot incidence
  ...
  ymax=max(inci)
  matplot(years,inci,ylab='',col=tree1.cor,type='l',lty=1:ncol(inci),log='y',lwd=2,xaxt='n',xpd=T)
  ys=as.numeric(unlist(inci[nrow(inci),]))
  y.diff=diff(ys)
  y.diff[which(y.diff<ymax/10)]=ymax/10
  ys=ys[1]+c(0,cumsum(y.diff))
  if(sex==2 & tr==1 & cut.off==.4) ys=c(.3,1,3,5.5,8,25)
  if(sex==2 & tr==2 & cut.off==.4) ys=c(3.5,5.5,15)
  # if(sex==2 & cut.off==.35) ys=c(.5,1.3,2.8,4,6,8.5,25)
  if(sex==1 &  tr==1 &  cut.off==.4) ys=c(.3,.6,2,3.2,5.3,7.6)
...
# Plot eFigures 3 (for men) and 4 (for women)
pdf(paste0(dir_proj,'fig_cluster.all_',method,'_cut',cut.off,'_',sexes[sex],'_a',min(ages),'-',max(ages),tag,'.pdf'),width = 6,height=7.5)
...

{
  ...
  tree1$labels=ori.labels # gsub("\\.",' ',tree1$labels) # format the cancer names
  nbranch=length(tree1$labels)
  col.edge=rep(cols2[tr%%2+1],nbranch);
  col.edge[tree1$order[which(tree1$height<cut.off)]]='red'
  
  tree1=as.dendrogram(tree1); # somehow the order is messed up
  labels_colors(tree1)=tree1.cor # reg.colors[tree1.cor]; # set the color
  labels(tree1)=ori.labels[ori.order];  # reset the labels in order
  # set(tree1,"branches_k_color",k=3,c('red','black','blue'))
  plot(tree1,horiz = T,edgePar = list(lwd=.5,col=cols2[tr%%2+1]),
       # edgePar = list(lwd=rep(.5,nbranch),col=col.edge),
       # edgePar = list(lwd=c(.5,.5,.5),col=c('grey','green','red')),
       # xlim=c(cut.off+.05,0),
       axes=F,xpd=T)
